        ...
        <div class="flex-1 flex justify-between md:hidden">
            @if ($paginator->onFirstPage())
                <span aria-disabled="true" aria-label="@lang('pagination.previous')"
                      ...
                      text-gray-700 bg-white hover:text-gray-500 focus:outline-none focus:shadow-outline-blue focus:border-blue-300
            ...
            @else
                <button wire:click="previousPage" rel="prev" aria-label="@lang('pagination.previous')"
                        class="relative inline-flex items-center px-4 py-2 border border-gray-300 text-sm leading-5 font-medium rounded-md
                        text-gray-700 bg-white hover:text-gray-500 focus:outline-none focus:shadow-outline-blue focus:border-blue-300 active:bg-gray-100
            ...
            @if ($paginator->hasMorePages())
                <button wire:click="nextPage" rel="prev" aria-label="@lang('pagination.next')"
                        ...
                        bg-white hover:text-gray-500 focus:outline-none focus:shadow-outline-blue focus:border-blue-300 active:bg-gray-100 active:text-gray-700
            ...
            <div>
                <span class="relative z-0 inline-flex shadow-sm">
                    @if ($paginator->onFirstPage())
                        ...
                        text-gray-500 hover:text-gray-400 focus:z-10 focus:outline-none focus:border-blue-300 focus:shadow-outline-blue active:bg-gray-100
                            ...
                            @foreach ($element as $page => $url)
                                @if ($page == $paginator->currentPage())
                                    <span aria-current="page"
                                          class="cursor-not-allowed -ml-px relative inline-flex items-center px-4 py-2 border border-gray-300 bg-white text-sm leading-5 font-medium text-indigo-700
                                            hover:text-gray-500 bg-indigo-100 focus:z-10 focus:outline-none focus:border-blue-300 focus:shadow-outline-blue active:bg-indigo-100 active:text-gray-700
                                            transition ease-in-out duration-150">
                                      {{ $page }}
                                    </span>
                                @else
                                    <button wire:click="gotoPage({{ $page }})" type="button"
                                            class="-ml-px relative inline-flex items-center px-4 py-2 border border-gray-300 bg-white text-sm leading-5 font-medium text-gray-700 hover:text-gray-500
